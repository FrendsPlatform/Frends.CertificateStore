...
namespace Frends.CertificateStore.Tests
{
    ...
    class TestClass
    {
        ...
        public static async Task TearDown()
        {
            ...
            var removal2 = await CertificateStore.RemoveFromLocalUser("login.yahoo.com");
            Assert.IsTrue(removal.Success);
            Assert.IsTrue(removal2.Success);
        }

        [Test]
        public static async Task ShouldAddRawCertificateDataToLocalUserStore()
        {
            var rawData = await ConvertBase64StringToValidRawCertificateData();
            var res = await CertificateStore.AddToLocalUserViaRawData(rawData.CertificateRawData, "password");
            Assert.IsNotNull(res);
        }

        [Test]
        public static async Task ShouldGetCorrectCertificateInUserStore()
        {
            ...
            var localCerts = await CertificateStore.GetAllCertificatesInSelectedStore(input);
        ...
        private static async Task<X509CertificateResultRaw> ConvertBase64StringToValidRawCertificateData()
        {
            ...
            var res = await CertificateStore.GetCertificateFromBase64(base64);
